...
program example
   ...
   mean = total/size(waterlevels_for_mean)
   ...
   print *, 'difference_from_mean() is the same as differences from loop:', &
      all(difference_from_mean(waterlevels_for_mean, mean) == differences)

   ! Note: this looks specific to waterlevels, but isn't
   ! Also, we need the mean *and* the standard deviation at once, so we should combine these values into a derived type
   print *, 'Standard deviation:', sqrt(sum(difference_from_mean(waterlevels_for_mean, mean)**2)/ &
