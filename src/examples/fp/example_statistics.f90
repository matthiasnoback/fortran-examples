...
program example
   ...
   ! Step 3: use foldl to sum reals (as an example of foldl)
   total = foldl(sum_reals_wp, 0.0_wp, waterlevels_for_mean)
   print *, 'Intrinsic sum is same as foldl sum: ', total == sum(waterlevels_for_mean)

   ! Note potential risk of dividing by 0
   old_mean = total / size(waterlevels_for_mean)
   ...
   do index = 1, size(waterlevels)
      if (waterlevels(index)%station_id == 1) then
         differences(waterlevels(index)%timestep) = waterlevels(index)%waterlevel - old_mean
      end if
   end do

   ! Step 4: use elemental function to map values to differences from mean
   print *, 'difference_from_mean() is the same as differences from loop:', &
      all(difference_from_mean(waterlevels_for_mean, old_mean) == differences)

   ! Step 5: Use Derived type to combine mean and standard deviation and call a factory function with an array of reals as input
   old_stddev = sqrt(sum(difference_from_mean(waterlevels_for_mean, old_mean)**2) / &
                     size(waterlevels_for_mean))
   new_stddev = mean_and_standard_deviation(waterlevels_for_mean)
   print *, 'Standard deviation is same when using DT:', old_stddev == new_stddev%standard_deviation
   print *, 'Mean is same when using DT:', old_mean == new_stddev%mean
