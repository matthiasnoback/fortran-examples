module model
   ...
   private
   public :: waterlevel_at_station_t
   public :: fill_waterlevels
   public :: average
...
program example
   use model, only: waterlevel_at_station_t, fill_waterlevels, average
   use iso_fortran_env, only: wp => real64
   ...
   do index = 1, size(waterlevels)
      if (waterlevels(index)%station_id == 1) then
         total = total + waterlevels(index)%waterlevel
         number_of_observations = number_of_observations + 1
      end if
   end do
   mean = total/number_of_observations
