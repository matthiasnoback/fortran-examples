module m_logging
   ...
   type, extends(t_abstract_logger) :: t_multi_logger
      class(t_abstract_logger), dimension(:), allocatable :: subloggers
   contains
...
contains
   ...
   subroutine multi_logger_log(self, message)
      class(t_multi_logger), intent(in) :: self
      character(len=*), intent(in) :: message

      integer :: i

      do i = 1, size(self%subloggers)
         call self%subloggers(i)%log(message)
      end do
...
program example
   use m_logging, only: init_logging, t_abstract_logger, t_do_everything_logger, t_multi_logger
   ...
   logger = t_multi_logger([t_do_everything_logger(), t_do_everything_logger()])
